...
/**
 * AuthProvider - Manages authentication state and permissions
 * 
 * This provider must receive a configured Supabase client from the consuming application.
 * It handles session management, user profiles, and permission loading.
 * 
 * @example
 * ```tsx
 * import { createSupabaseClient, AuthProvider } from '@core-erp/entity'
 ...
 * ```
 */
export function AuthProvider({ children, supabaseClient, toast }: AuthProviderProps) {
  ...
  // Default no-op toast if not provided
  ...
  const handleExternalLogout = useCallback(() => {
    ...
    toastHandler.info('You have been signed out from another tab.')
  }, [toastHandler])
  ...
  }, [fetchUserPermissions, supabaseClient, toastHandler])
  ...
  const signOut = async () => {
    try {
      ...
      toastHandler.error('Failed to sign out. Please try again.')
    }
  }
  ...
  useEffect(() => {
    if (!isOnline && session) {
      toastHandler.warning('You are offline. Some features may not be available.')
    } else if (isOnline && session && isReconnecting) {
      toastHandler.success('Back online. Connection restored.')
    }
  }, [isOnline, isReconnecting, session, toastHandler])
